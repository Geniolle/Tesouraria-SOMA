    ...
    def _load_reference_data(self) -> list[list]:
        try:
            result = self.sheets_client.service.spreadsheets().values().get(
                ...
                range=self.sheets_client.get_data_range(self.spreadsheet_id, self.reference_sheet),
            ).execute()
    ...
    def _find_match(self, description: str) -> Optional[dict]:
        ...
        for ref_row in self.ref_data:
            ...
            if (desc_norm in ref_texto_norm) or (ref_texto_norm in desc_norm):
                ...
                doc_soma = (
                    str(ref_row[doc_soma_idx]).strip()
                    if doc_soma_idx is not None and doc_soma_idx < len(ref_row)
                    else ""
                )
                desc_soma = (
                    str(ref_row[desc_soma_idx]).strip()
                    if desc_soma_idx is not None and desc_soma_idx < len(ref_row)
                    else ""
                )
